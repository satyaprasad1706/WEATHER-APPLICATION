...
let unit = "metric", lastWeather = null, lastForecast = null;
let map = null, mapMarker = null, weatherTileLayer = null, mapInitialized = false;

function sanitize(str) { const d = document.createElement("div"); d.textContent = String(str ?? ""); return d.innerHTML; }
function validateCity(input) { const t = input.trim().slice(0, 100); return /^[a-zA-Z\u00C0-\u024F\s'\-,.]+$/.test(t) ? t : null; }
function debounce(fn, ms) { let t; return (...args) => { clearTimeout(t); t = setTimeout(() => fn(...args), ms); }; }
function toDisplayTemp(c) { return unit === "metric" ? Math.round(c) : Math.round(c * 9 / 5 + 32); }
function tempSym() { return unit === "metric" ? "°C" : "°F"; }
function windDisplay(ms) { return unit === "metric" ? `${ms} m/s` : `${(ms * 2.237).toFixed(1)} mph`; }

// ── Clock ──
setInterval(() => { document.getElementById("headerTime").textContent = new Date().toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit", second: "2-digit" }); }, 1000);
document.getElementById("headerTime").textContent = new Date().toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit", second: "2-digit" });
...
// ── Unit Toggle ──
btnC.addEventListener("click", () => { if (unit === "metric") return; unit = "metric"; btnC.classList.add("active"); btnF.classList.remove("active"); if (lastWeather) renderWeather(lastWeather); if (lastForecast) renderForecast(lastForecast); });
btnF.addEventListener("click", () => { if (unit === "imperial") return; unit = "imperial"; btnF.classList.add("active"); btnC.classList.remove("active"); if (lastWeather) renderWeather(lastWeather); if (lastForecast) renderForecast(lastForecast); });

// ── History ──
function getHistory() { try { return JSON.parse(localStorage.getItem("wx_history") || "[]"); } catch { return []; } }
function saveHistory(city) { let h = getHistory().filter(c => c.toLowerCase() !== city.toLowerCase()); h.unshift(city); localStorage.setItem("wx_history", JSON.stringify(h.slice(0, 6))); renderHistory(); }
function renderHistory() {
  const chips = document.getElementById("historyChips"); chips.innerHTML = "";
  getHistory().forEach(city => {
    ...
    const delI = document.createElement("i"); delI.className = "fa fa-xmark"; del.appendChild(delI);
    del.addEventListener("click", e => { e.stopPropagation(); localStorage.setItem("wx_history", JSON.stringify(getHistory().filter(c => c !== city))); renderHistory(); });
    chip.appendChild(ico); chip.appendChild(label); chip.appendChild(del); chips.appendChild(chip);
  });
}
renderHistory();
...
// ── Helpers ──
function getTip(id, humidity, wind) {
  ...
  if (id === 800 && wind > 10)  return "☀️ Clear skies but windy! Great for a kite day.";
...
function windDegToDir(deg) { return ["N","NE","E","SE","S","SW","W","NW"][Math.round(deg/45)%8]; }
...
async function fetchAQI(lat, lon) {
  const badge = document.getElementById("aqiBadge"), dot = document.getElementById("aqiDot");
  const aqiText = document.getElementById("aqiText"), bar = document.getElementById("aqiDetailBar");
  badge.style.display = "";
  try {
    const res = await fetch(`${AQI_URL}?lat=${lat}&lon=${lon}&appid=${API_KEY}`);
    if (!res.ok) throw new Error();
    const data = await res.json(), entry = data?.list?.[0];
    ...
    bar.style.display = "";
  } catch { badge.style.display = "none"; bar.style.display = "none"; }
}
...
function renderForecast(list) {
  ...
  list.forEach(item => {
    ...
    daily[day].temps.push(item.main.temp); daily[day].icons.push(item.weather[0].icon); daily[day].descs.push(item.weather[0].description);
  });
  ...
  const sym = tempSym(), container = document.getElementById("forecastList"); container.innerHTML = "";
  rows.forEach(([day, data]) => {
    const hi = toDisplayTemp(Math.max(...data.temps)), lo = toDisplayTemp(Math.min(...data.temps));
    const icon = data.icons[Math.floor(data.icons.length/2)], desc = data.descs[Math.floor(data.descs.length/2)];
    ...
    const img = document.createElement("img"); img.className = "forecast-icon"; img.src = `https://openweathermap.org/img/wn/${sanitize(icon)}@2x.png`; img.alt = desc;
...
function renderWeather(d) {
  const icon = d.weather[0].icon, id = d.weather[0].id, sym = tempSym();
  setTheme(id, icon);
  ...
  document.getElementById("dewPoint").textContent    = `${toDisplayTemp(calcDewPoint(d.main.temp, d.main.humidity))}${sym}`;
  const dirEl = document.getElementById("windDir"); dirEl.textContent = "";
  const arrow = document.createElement("i"); arrow.className = "fa fa-arrow-up"; arrow.style.transform = `rotate(${d.wind.deg??0}deg)`;
  dirEl.appendChild(arrow); dirEl.appendChild(document.createTextNode(` ${windDegToDir(d.wind.deg??0)}`));
  ...
  errorDiv.classList.add("hidden"); mainContent.classList.remove("hidden");
  if (map) { map.setView([d.coord.lat, d.coord.lon], 10); placeMapMarker(d.coord.lat, d.coord.lon); }
}

// ── Fetch ──
async function fetchAll(rawCity) {
  const city = validateCity(rawCity); if (!city) { showError("Invalid city name."); return; }
  showLoader();
  try {
    const res = await fetch(`${BASE_URL}?q=${encodeURIComponent(city)}&appid=${API_KEY}&units=metric`);
    if (!res.ok) { const j = await res.json().catch(()=>({})); throw new Error(res.status===404 ? "City not found. Please try again." : `Error: ${j.message||res.status}`); }
    const data = await res.json(); lastWeather = data;
    renderWeather(data); saveHistory(data.name);
    fetchAQI(data.coord.lat, data.coord.lon); fetchForecastByCoords(data.coord.lat, data.coord.lon);
...
async function fetchAllByCoords(lat, lon) {
  ...
  try {
    ...
    const data = await res.json(); lastWeather = data;
    renderWeather(data); saveHistory(data.name);
    fetchAQI(data.coord.lat, data.coord.lon); fetchForecastByCoords(lat, lon);
  } catch { showError("Could not fetch weather for this location."); } finally { hideLoader(); }
}
...
async function fetchForecastByCoords(lat, lon) {
  try {
    ...
    if (!res.ok) throw new Error();
    const data = await res.json(); lastForecast = data.list; renderForecast(data.list);
  } catch { document.getElementById("forecastList").textContent = "Forecast unavailable."; }
}

function showError(msg) { mainContent.classList.add("hidden"); errorDiv.classList.remove("hidden"); errorDiv.querySelector("span").textContent = msg || "Something went wrong."; }
...
function initMap() {
  ...
  L.tileLayer("https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png", { attribution:'© <a href="https://carto.com/">CARTO</a>', maxZoom:18 }).addTo(map);
  ...
  map.on("click", async (e) => { placeMapMarker(e.latlng.lat, e.latlng.lng); await showMapPopup(e.latlng.lat, e.latlng.lng); });
  document.querySelectorAll(".map-layer-btn").forEach(btn => {
    btn.addEventListener("click", () => {
      document.querySelectorAll(".map-layer-btn").forEach(b => b.classList.remove("active")); btn.classList.add("active");
...
function placeMapMarker(lat, lng) {
  ...
  mapMarker = L.marker([lat, lng], { icon: L.divIcon({ className:"", html:`<div style="width:14px;height:14px;background:#60a5fa;border:3px solid #fff;border-radius:50%;box-shadow:0 0 10px rgba(96,165,250,0.8);"></div>`, iconSize:[14,14], iconAnchor:[7,7] }) }).addTo(map);
}

async function showMapPopup(lat, lng) {
  if (mapMarker) mapMarker.bindPopup(`<div class="map-popup"><div class="map-popup-loader"><i class="fa fa-spinner fa-spin"></i> Loading...</div></div>`, { maxWidth:280 }).openPopup();
  try {
    ...
    const w = await wRes.json(), aqiData = aqiRes.ok ? await aqiRes.json() : null;
    const sym = tempSym(), icon = sanitize(w.weather[0].icon), desc = w.weather[0].description;
    const aqiLabels = ["","Good","Fair","Moderate","Poor","Very Poor"];
    const aqiColors = ["","#22c55e","#84cc16","#eab308","#f97316","#ef4444"];
    const aqiVal = aqiData?.list?.[0]?.main?.aqi;
    const aqiHtml = aqiVal ? `<div class="map-popup-aqi" style="background:${aqiColors[aqiVal]}22;border-color:${aqiColors[aqiVal]}55;color:${aqiColors[aqiVal]}"><i class="fa fa-leaf"></i> Air Quality: ${aqiLabels[aqiVal]}</div>` : "";
    const html = `<div class="map-popup">
      ...
      <div class="map-popup-row"><img class="map-popup-icon" src="https://openweathermap.org/img/wn/${icon}@2x.png" alt="${sanitize(desc)}"/>
        <div><div class="map-popup-temp">${toDisplayTemp(w.main.temp)}${sym}</div><div class="map-popup-desc">${sanitize(desc)}</div></div>
      </div>
  ...
  } catch { if (mapMarker) mapMarker.setPopupContent(`<div class="map-popup"><div class="map-popup-loader">Could not load weather for this location.</div></div>`); }
}
...
locationBtn.addEventListener("click", () => {
  ...
  navigator.geolocation.getCurrentPosition(p => fetchAllByCoords(p.coords.latitude, p.coords.longitude), () => alert("Unable to retrieve your location."));
});
